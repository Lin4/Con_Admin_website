...
$url = 'http://primetech.digisarathi.net/contructionapi.php/api/compliance/list/'.$_SESSION['username'].'/'.$project_id;
...
$project_report="";
if(!empty($responseArray['complianceForm'])){
foreach($responseArray['complianceForm'] AS $response){
	$project_report .= "<tr>
			<td>".$response['complianceReportNo']."</td>
			<td>".date('Y-m-d', strtotime($response['DateIssued']))."</td>
			<td>".$response['PrintedName']."</td>
			<td>
				<a href='index.php?page_name=edit-compliance-report&action=edit&cnno=".$response['complianceReportNo']."&pid=".$project_id."' class='btn btn-sm btn-info'>View Report</a>
			</td>
			</tr>";
}
}
...
<ul class="nav nav-tabs" role="tablist">
  <li class="active"><a href="index.php?page_name=compliance-reports&pid=<?php echo $project_id; ?>">Compliance Report</a></li>
