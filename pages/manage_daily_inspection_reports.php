...
$url = 'http://primetech.digisarathi.net/contructionapi.php/api/dailyinspection/list/'.$_SESSION['username'].'/'.$project_id;
...
$project_report="";
if(!empty($responseArray['dailyInspectionForm'])){
foreach($responseArray['dailyInspectionForm'] AS $response){
	$project_report .= "<tr>
			<td>".$response['dailyInspecNo']."</td>
			<td>".date('Y-m-d', strtotime($response['Date']))."</td>
			<td>".$response['printedName']."</td>
			<td><a href='index.php?page_name=edit-daily-inspection-report&action=edit&editId=".$response['id']."&inno=".$response['dailyInspecNo']."&pid=".$project_id."' class='btn btn-info btn-sm'>View Report</a></td>
			</tr>";
}
}
?>
...
<ul class="nav nav-tabs" role="tablist">
  ...
  <li class="active"><a href="index.php?page_name=daily-inspection-reports&pid=<?php echo $project_id; ?>">Daily Inspection Report</a></li>
...
<br>
<legend>Daily Inspection Report</legend>
