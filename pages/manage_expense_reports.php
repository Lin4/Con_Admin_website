...
$url = 'http://primetech.digisarathi.net/contructionapi.php/api/expense/list/'.$_SESSION['username'].'/'.$project_id;
...
$project_report="";
if(!empty($responseArray['expenseReportModel'])){
foreach($responseArray['expenseReportModel'] AS $response){
	$project_report .= "<tr>
			<td>".$response['eXReportNo']."</td>
			<td>".date('Y-m-d', strtotime($response['Date']))."</td>
			<td>".$response['EMPName']."</td>
			<td><a href='index.php?page_name=edit-expense-report&action=edit&exno=".$response['eXReportNo']."&pid=".$project_id."' class='btn btn-info btn-sm'>View Report</a></td>
			</tr>";
}
}
?>
...
<ul class="nav nav-tabs" role="tablist">
  ...
  <li class="active"><a href="index.php?page_name=expense-reports&pid=<?php echo $project_id; ?>">Expense Report</a></li>
